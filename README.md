...
The simulation view shows two drawings and says which is which. On the right, the local simulation runs a built-in `APRENDIZ SimArm-6` trajectory, labelled as internal and captioned with the robot it draws rather than with the task the person typed. On the left, when a motion analysis exists, the movement observed in the video is drawn from its own samples: one angle-over-time track per joint, with a shared playhead, the video timestamps it spans, and the live angle beside each joint name. A track breaks wherever the joint stopped being visible, and scrubbing into a break reports no reading instead of an interpolated number. No figure is reconstructed, because the samples carry angles and no geometry; sending those angles to the robot still needs an imported ARP-1 profile and a human-authored joint map, which the panel beneath it states.
