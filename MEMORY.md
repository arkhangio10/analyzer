...
Never bake credentials, API keys, `.env` contents, user uploads, or private training data into the Docker image. Supply secrets and user-specific configuration at runtime. The export system is implemented (2026-09-08). `POST /api/projects/{id}/agent-packages` builds a package from the latest approved procedure and `GET .../{package_id}/download` returns it as a zip. The exported agent is this application, pinned to the versions that produced the export through `requirements.lock`, with `skill/skill.json` baked in and every provider call disabled. What the agent is not is written as `Literal[False]` fields on the skill, so a package that claims more fails validation instead of being served.
...
### 2026-08-30 — Make the finishing order executable and verify Compose persistence

- **Decision:** Replace the informal finishing list with `docs/backlog.md`, whose tasks carry explicit gates and acceptance evidence, and run the unit, browser, and hardened-container checks in a provider-disabled GitHub Actions workflow.
- **Reason:** The prior plan counted more tasks than it named, mixed human decisions with unattended work, and left the newest persistence guarantees unverified inside the actual container. A local server already occupied `127.0.0.1:8080`, so Compose also needed a configurable host port.
- **Alternatives:** Continue tracking dependencies in a generated PDF, assume the host tests proved the image, or defer automation until deployment.
- **Impact:** The backlog now distinguishes ready work from human, cost, evidence, simulator, deployment, and hardware gates. The rebuilt image runs as UID/GID 10001 with a read-only root, includes the protected frozen cases, reports `durable_storage: true`, and retained a project across a real container restart in the `aprendiz-data` volume. `APP_PORT` can move the host binding without changing container port 8080. CI keeps Gemini and YouTube search disabled while checking both Python suites and these container boundaries.

### 2026-08-30 — Persist workflow evidence without retaining private action values

- **Decision:** Store robot-motion sessions and evaluations, computer practices, sandbox executions, and browser executions as schema-versioned records under `DATA_DIR/records/`. Persist only already-redacted execution results; remove typed browser values and URL queries from pending-practice records, and reload such a draft blocked so it must be created and approved again.
- **Reason:** Projects and paid extraction evidence already survived restarts, but the practice and execution path did not. Persisting raw typed values merely to make a draft resumable would conflict with the product's evidence and secret boundaries.
- **Alternatives:** Keep the remaining services in memory, persist every request verbatim, or silently resume a redacted draft with incomplete actions.
- **Impact:** The workflow evidence can be audited after a restart without storing file contents, typed values, or URL queries. `/api/status` now separately reports `workflow_evidence_durable`. Adaptation and motion-retarget explanations also follow the requested `es` or `en` language; changing the interface language refreshes adaptation output without executing anything.

### 2026-09-06 — Ask ClickHouse the audit questions, with one copy of the rules

- **Decision:** Keep the Python audit as the reference and add the same four checks as window-function SQL in ClickHouse. Both paths compute counts and hand them to one shared `verdict_from_counts`; a differential test inserts identical samples, asks both, and asserts the same verdict and the same counts.
- **Reason:** One short clip at 4 fps is already a couple of hundred timestamped readings and a library is millions, which is a columnar workload. Reimplementing the rules in SQL would let the two drift apart silently, and a partner integration that no test depends on is decorative.
- **Alternatives:** Audit only in Python and use ClickHouse as dumb storage; reimplement the rules in SQL and compare by eye; skip the columnar store entirely.
- **Impact:** The differential test is what makes the integration load-bearing, and it immediately earned its place: `lagInFrame` requires its default to carry the argument's exact type, `sign()` returns `Int8`, and the literal `0` is `UInt8`, so the acyclic-joint query had never run against a real server. It failed with `BAD_ARGUMENTS`, `audit_from_sql` degraded to `None`, and nothing said so because the tests skipped without a configured server. Fixed with `toInt8(0)`, which leaves the meaning alone.

### 2026-09-06 — Keep records and uploads where a stateless container can reach them

- **Decision:** Add `GcsRecordStore` honouring the existing `RecordStore` protocol, and split video storage into a `VideoStorage` base with local and bucket backends. `GCS_BUCKET` is the only switch: setting it moves both records and uploads. Widen the eight services from `JsonRecordStore | None` to `RecordStore | None`.
- **Reason:** Cloud Run has no disk that survives a revision, so `JsonRecordStore`'s directory has nowhere to live there. The protocol existed from the earlier refactor but nothing could use it, because every service annotated the concrete class.
- **Alternatives:** Mount a filesystem; keep records in ClickHouse alongside the evidence; change every caller when the destination changes.
- **Impact:** `stored_locally`, permanently true, became `storage_location` with no default, so the code must report what the backend really did: `this_machine` or `your_bucket`. `sent_to_provider` stays permanently false — that is the guarantee that matters and it does not vary by destination. The shared base holds what must be identical either way (what is accepted, how the stream is capped and hashed, what the record claims), and a test compares a local and a bucket record field by field, permitting a difference only in the location, so a guarantee cannot weaken on the way to the cloud. Both backends are tested against doubles: that shows the clients are used correctly and nothing about how Cloud Storage really behaves.

### 2026-09-06 — Compose the QC pipeline by hand, because ADK's gate does not gate

- **Decision:** Implement the QC pipeline as six ADK agents (ingest, extraction, approval gate, motion, audit, report) but write the sequential composition by hand rather than using `SequentialAgent`. Check `requires_approval` in the stage base class.
- **Reason:** Measured before building on it: a sub-agent that sets `escalate` does **not** stop a `SequentialAgent` — every later sub-agent still runs; `LoopAgent` is what `escalate` terminates. A human approval gate built on it would read as enforced and would not be, which is the worst failure available for a governance control.
- **Alternatives:** Use `SequentialAgent` with `escalate` and hope; check approval in each stage separately; use `google.adk.workflow.Workflow`, the non-deprecated replacement, which is a `BaseNode` rather than a `BaseAgent` and does not compose as a sub-agent yet.
- **Impact:** A test pins the ADK behaviour that forced the choice, so a future version changing it says so out loud. The gate lives in the base class, so a stage added later inherits it, and a test walks the pipeline rather than naming stages one at a time. This is the second line, not the first: `MotionAnalysisService` already refuses an unapproved procedure where the cloud call would be spent, and that check guards the money whether or not anyone runs the pipeline. No stage initiates a provider call, asserted by a double that fails the test if it is ever asked to analyse. The boundaries that raise `NotImplementedError` stay; the pipeline orchestrates existing evidence rather than superseding them.

### 2026-09-06 — Show the counts a verdict came from, and start writing evidence

- **Decision:** Add the `/qc` console: a library view, an asset view carrying the audit's counts beside an SVG chart of the retained samples, and a button that recomputes the verdict from those samples. Wire `EvidenceStore` into `MotionAnalysisService` so completed analyses reach ClickHouse.
- **Reason:** `EvidenceStore` was built, tested, and never wired — only the tests ever inserted a row, so a library view sourced from the database would have been empty for good. And the claim being made is that a verdict is recomputable; a reader cannot check a verdict, only the arithmetic behind one.
- **Alternatives:** Show verdicts alone; add the console to the existing product surface; have the console commission a new analysis when none exists.
- **Impact:** A refusing evidence store does not fail an analysis: the call is already paid for and already retained by then, so losing the columnar copy must not undo it. An absent ClickHouse does not produce an empty library, which would read as a clean one — the view reports `evidence_available` false, says which part is missing, and still lists what the local record store holds. `QcFindingView.values` stays `dict[str, str]` matching `MotionAuditFinding`: a first pass typed it as floats and broke on a visibility reading `partial`, and the values are carried as data precisely so an interface can restate a finding in the reader's language. The chart is drawn as SVG from the series, so the page loads no third-party script. The robot track, the setup wizard and the bilingual toggle stay on the product surface, held off by a browser test.

### 2026-09-06 — Stop reporting a writable directory as durability on Cloud Run

- **Decision:** Read `K_SERVICE`, which Cloud Run sets in every container it starts, and separate `durable_storage` (a store accepted the write) from `records_survive_restart` (the write outlives the container) in `/api/status`. Log an error at startup when the second is false.
- **Reason:** On Cloud Run a writable container filesystem accepts records and loses them at the next revision or scale event, while reporting itself perfectly durable. Silently losing records is worse than refusing them. Verified: with `GCS_BUCKET` set the process touches no disk at startup; without it, it creates the data directory.
- **Alternatives:** Refuse to start without a bucket; document the constraint and leave the status endpoint claiming durability; treat any writable directory as durable.
- **Impact:** `deploy/deploy.sh` creates the bucket, service account, secret, registry and service idempotently, reading the ClickHouse password from stdin and piping it to Secret Manager so it never becomes an argument. Permissions are scoped — `objectAdmin` on the bucket rather than the project, `secretAccessor` per secret — and no Vertex AI role is granted at all, because provider calls are disabled by default and an account that cannot call a model cannot be made to spend money by a bug. Chromium became optional via `INSTALL_BROWSER`, since it is only reachable when `COMPUTER_BROWSER_ENABLED` is true and that is false by default. **None of the deployment has been run:** no Google Cloud resource has been created and the image has not been built.

### 2026-09-08 — Build the per-agent Docker export ahead of the P2.6 gate

- **Decision:** Implement P4.1 now, as a self-contained package whose agent is this application with the approved skill baked in, rather than waiting for P2.6 (one unseen case proven end to end).
- **Reason:** The owner asked for the product flow to end where the documented flow ends — a runnable Docker delivery — and the last mile was the only part of that flow the repository described but did not have. Building the agent as the application itself keeps one runtime, one set of typed guarantees, and a reproducible build from `requirements.lock`.
- **Alternatives:** Wait for P2.6 as the backlog required; deliver a loose directory of records instead of a package; write a separate lightweight agent runtime.
- **Impact:** `AgentExportService` is deterministic: the same records and the same `created_at` give the same bytes and package id, and a download after a restart rebuilds and checks the digests rather than serving a mismatch. The adaptation plan is recomputed with a random id on every call, so the packaged copy is identified by what it was built from. A rebuild names its own extraction, so approving a later version does not strand an older package. Agent mode (`AGENT_MODE`, `SKILL_PATH`) serves the agent page at `/`, the skill at `/api/skill` and a status at `/api/agent`; a skill that breaks its guarantees is refused with 503. Verified on 2026-09-08 with a real package: the launcher built and started it, `/health` answered in 4 s, the container ran as UID 10001 on a read-only filesystem with no `.env` and both frozen cases. Instructor examples remain unattached to extractions, so `skill.examples` ships empty rather than fabricated; the exported agent knows the procedure and is not authorised to execute it.

### 2026-09-07 — Draw the video's own movement, and never draw a body

- **Decision:** Add an observed-motion panel to the simulation view that draws one angle-over-time track per observed joint, built server-side by `build_observed_motion_preview` and served free at `GET /api/projects/{id}/video-procedures/{extraction_id}/motion-analysis/preview`. Do not reconstruct a figure from the samples.
- **Reason:** The owner analysed a video of a cheetah running, pressed simulate, and saw a six-axis arm. The arm was correct — the local simulation has always run a built-in trajectory — but the interface offered no drawing of the movement that actually came from the video, so the built-in one was the only thing on screen to attribute the video to. The samples do support one honest picture: each joint's own angle over time.
- **Alternatives:** Draw a stick figure from the angles; animate the observed angles on the built-in arm; leave the panel absent and rely on the text disclaimer.
- **Impact:** `ObservedMotionPreview` carries three permanent `Literal[False]` fields — `physically_measured`, `reconstructed_body_geometry`, `interpolated_across_gaps` — because a set of angles carries no link lengths, no joint positions, and no statement of what connects to what, so a silhouette drawn from them would be invented geometry displayed as evidence. A stroke breaks whenever the previous reading was occluded, or whenever the wait since it exceeds 2.5x that joint's own median spacing, and scrubbing into a break reports no reading rather than an interpolated number. Tracks are capped at twelve, ordered by range, and the omitted count is reported. Rejected: animating the observed angles on the built-in arm, which is the same category error `motion_retarget` already refuses — a quadruped's joints have no defined correspondence in a six-axis arm without an imported ARP-1 profile and a human-authored joint map.

### 2026-09-09 — The motion audit rejected a second video, so the panel must say so

- **Decision:** Keep drawing rejected samples in the observed-motion panel, but never under the badge that means "measured from your video". The panel now reads `evidence_verdict` and, when the audit refused the samples, changes its tag, restates its summary without the claim of provenance, recolours the traces, and prints why.
- **Reason:** Verifying the deployed cheetah analysis (`prj_310c1aadbff9` / `vpr_426b0d0819c5`) showed the panel shipped the day before was presenting a rejected sample set as evidence from the user's own footage. The analysis returned 290 samples across 8 joints of a `quadrupedal_locomotion` chain at mean confidence 0.75 — and failed on three counts: left and right carried an identical angle in 145 of 145 paired readings; all 290 samples reported visibility `clear`, so occlusion was never judged per frame; and every one of the 8 joints traced a single rise and fall across the window, when a running quadruped repeats its gait several times per second. The response also hit its output ceiling after 290 samples.
- **Alternatives:** Hide the panel when the audit fails, which would have removed the only place a person can see *what* the model fabricated; or leave the panel as shipped and rely on the verdict shown in the Video view, which is a different screen.
- **Impact:** This is the second failure of the same kind, on a different subject, a different chain, and a different video from the 2026-08-30 human-walking run. One failure was a bad response; two is a property of asking a vision model for joint angles. `P3.2` — benchmarking a real pose source against human-labelled landmarks — is therefore the blocker, not `P3.1`. Building the ARP-1 profile import and the human-authored joint map first would connect a robot to numbers that are not observations. The retarget verdict already refuses on exactly this ground: "These samples did not survive the plausibility audit, so there is nothing here to retarget."

### 2026-09-09 — Narrow to human demonstration, and measure instead of asking

- **Decision:** Build P3.2 as a local pose-model path over uploaded video, narrowed to human subjects by owner decision, rather than continuing to ask a vision model for joint angles. Add the anatomical range check the audit never had. Score measurements against human-labelled landmarks, and return `unvalidated` when nobody has labelled the video.
- **Reason:** Asking a language model for joint angles failed its deterministic audit twice, on two subjects and two chains. The failure is in the method, not the run. A pose model is a different kind of thing: it reports per-landmark confidence it did not choose, it is deterministic over the same file, and it cannot answer with a plausible narrative because it does not produce narratives. Narrowing to a human body is what makes this buildable at all — a 33-landmark human topology has no meaning on a quadruped, and animal pose estimation is a research problem rather than an engineering one.
- **Alternatives:** More frames or a better prompt on the vision path, which two failures say will not help; a full 3D lift from a single camera, whose depth is the least trustworthy thing one camera produces; building P3.1's joint map first, which would have connected a robot to numbers that are not observations.
- **Impact:** The measured path spends nothing (`cloud_calls_made` is `Literal[0]`, `sent_to_provider` `Literal[False]`) and its samples are the same `ObservedJointAngle` the vision path returns, so the audit that rejected the fabricated data is the audit this path has to survive. `physically_measured` stays false: a better estimator is still an estimator. Angles are read in the image plane and `measured_in_image_plane` says so, because a single camera's depth is not worth trusting and the cost is that the camera must see the movement side on. Normalised coordinates are rescaled by the frame aspect ratio before any angle is taken; skipping that tilts every joint on ordinary 16:9 footage. Readings below the visibility floor and readings outside human range are dropped and counted, never delivered with a caveat. `mediapipe` and `opencv-python-headless` are an optional `pose` extra, kept out of the base dependencies so every exported agent does not carry a pose model it has no use for. **Partly done.** The model was fetched and pinned on 2026-09-09 (`pose_landmarker_lite.task`, 5,777,746 bytes, sha256 `59929e1d...d574a`); the pin is trust on first use and the script says so. Five integration tests run it for real, including the negative control that matters: three seconds of random noise returns no landmarks, so the source does not invent a body where there is none. **Still not done:** no human-labelled case exists, so the benchmark has never run against ground truth and no real person has been measured. MediaPipe is an opt-in Docker build arg (`INSTALL_POSE`, default false), so the deployed image cannot measure until a deployment asks for it.
...
- a simulation view that draws two things and never lets them be read as one: on the right the built-in `APRENDIZ SimArm-6` trajectory, tagged as internal and captioned with the robot rather than the person's task; on the left, when a motion analysis exists, the movement observed in the video as one angle-over-time track per joint, with a shared playhead, the video timestamps it spans, and strokes that break wherever the joint stopped being visible. Nothing reconstructs a body from angles, and a rejected audit verdict restyles the panel and withdraws its claim of provenance instead of decorating it;
- a frozen evaluation set that currently holds two cases, both robot-motion and both `authored_by: specification`. No case on the video path exists, and none is human-authored, so `counts_as_external_validation` is false everywhere;
- initial Pydantic contracts for task definitions, procedures, skills, training examples, and evaluation results;
- typed robot-motion contracts, deterministic safety validation, movement procedure extraction, instructor-grounded replay evaluation, and durable versioned session/evaluation retrieval;
...
- dependency/configuration manifests, a versioned gated backlog, provider-disabled CI, secret-safe environment template, ignore rules, Dockerfile, Compose service, and smoke tests;
...
- Firestore and Pub/Sub: `FirestoreService` and `PubSubService` exist and are exported from `app.services`, but nothing in `app/api/runtime.py` constructs them, so neither is in use;
- browser execution in production: the deployed service runs with `COMPUTER_BROWSER_ENABLED=false`, so the computer path is reviewable but not executable there.

No longer on this list, because all three are built, deployed and verified: Cloud Storage record durability, Cloud Run, and per-agent Docker export packaging.
...
Deployed state verified on 2026-09-09 against Cloud Run revision `aprendiz-00013-sqt`, which serves 100% of traffic at `https://aprendiz-963953909890.us-central1.run.app`. That deployment has Vertex AI enabled, records durable in Cloud Storage (`GCS_BUCKET` set, `/api/status` reports `durable_storage: true`), the spend token and the ClickHouse password injected from Secret Manager, ClickHouse enabled for the QC audit, and browser execution disabled. The application-enforced spending ceiling is real and running: 20.00 PEN for the month, 0.036 PEN spent across 5 paid calls, priced at 0.36 and 1.44 PEN per million input and output tokens. A GCP budget was rejected as the mechanism because a budget alerts and does not cap.
...
Resolved on 2026-08-30: adaptation `missing_evidence` and blocked reasons now
use the language requested by the interface, and motion retarget uses the
language acknowledged for that analysis. The audit findings remain
language-neutral data with localized explanations.

On 2026-08-30 the four items that needed nothing from the operator landed: local
video intake (D1), version history and diffs (C6), project reconciliation with an
independence verdict (B4), and browser coverage (I1). Three things are worth
remembering.

The browser suite must stay out of the default `pytest` run. Playwright's sync
API keeps an event loop alive for the life of its fixtures, and every later test
calling `asyncio.run` then fails with "cannot be called from a running event
loop". `testpaths` is therefore `tests/unit`, and the browser suite runs as
`pytest tests/browser`.

Writing that suite found a real interface bug: selecting the "Local video"
source option cleared `currentProject`, so a video could never be attached to a
project opened from saved work. Choosing YouTube or automatic search still
clears it, because those redefine the source that gets extracted; an upload is
extra evidence beside the project, so it must not discard it.

Uploads are never sent to a provider. That is a decision the operator did not
explicitly make, so the local-only default was chosen and written into the
contract (`sent_to_provider` is permanently false). If sending an uploaded file
to Vertex is ever wanted, it needs the operator's explicit agreement first.
