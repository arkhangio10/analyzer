...
- a *verified* pose measurement pipeline. The vision-model path failed its evidence audit twice, on two subjects and two kinematic chains (2026-08-30 human walking, 2026-09-07 quadrupedal locomotion); treat that as a property of asking a vision model for joint angles, not a bad run to retry. The replacement — a local pose model over uploaded frames, narrowed to human subjects — is built and tested but has never measured a real video: the pinned model file is not fetched and no human-labelled benchmark case exists. Nothing on that path has been validated against ground truth yet;
